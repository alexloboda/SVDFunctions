...
filterGmatrix <- function(gmatrix, imputationResults, minVariantCallRate = 0.95,
  ...
  list(genotype = gmatrix, predicted = imputationResults)
}
...
prepareInstance <- function(
  ...
  salt <- writeYaml(clusterResults, clusters, variants = rownames(gmatrix), 
            outputFileName = outputFileName, title = title)

  if (!is.null(keptSamplesFile)) {
    saveRDS(cluster_ids, keptSamplesFile)
  }
  cluster_ids
}
